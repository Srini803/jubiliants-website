    ...
    <>
      ...
      <button
        ...
        style={{
          ...
          background:"linear-gradient(135deg,#2E3192,#23276F)",
          border:"none", cursor:"pointer",
          boxShadow:"0 4px 20px rgba(46,49,146,0.45)",
      ...
      >
        {open
          ...
          : <svg width="26" height="26" fill="none" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              ...
              <circle cx="8.5" cy="11" r="1.2" fill="#2E3192"/>
              <circle cx="12" cy="11" r="1.2" fill="#2E3192"/>
              <circle cx="15.5" cy="11" r="1.2" fill="#2E3192"/>
            </svg>
        }
      </button>
      ...
      {/* Unread dot */}
      {!open && (
        <span style={{position:"fixed",bottom:"2.75rem",right:"1.5rem",zIndex:1001,width:"10px",height:"10px",borderRadius:"50%",background:"#22C55E",border:"2px solid #FAF7F0"}} />
      )}
      ...
      {open && (
        <div style={{
          ...
          background:"#FFFFFF", border:"1px solid rgba(35,39,111,0.15)",
          borderRadius:"16px", boxShadow:"0 24px 60px -12px rgba(25,28,82,0.35)",
          display:"flex", flexDirection:"column", overflow:"hidden",
        }}>
          {/* Header */}
          <div style={{background:"linear-gradient(135deg,#23276F,#2E3192)",padding:"1rem 1.25rem",display:"flex",alignItems:"center",gap:"0.75rem"}}>
            <div style={{width:"36px",height:"36px",borderRadius:"50%",background:"rgba(255,255,255,0.15)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"1.1rem",color:"#FBB040"}}>✦</div>
            <div>
          ...
          <div style={{flex:1,overflowY:"auto",padding:"1rem",display:"flex",flexDirection:"column",gap:"0.75rem",maxHeight:"360px"}}>
            {messages.map((m,i)=>(
              <div key={i} style={{display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start"}}>
                <div style={{
                  ...
                  background:m.role==="user"?"#2E3192":"#F2EDE1",
                  color:m.role==="user"?"white":"#2A2D55",
            ...
            {loading && (
              <div style={{display:"flex",justifyContent:"flex-start"}}>
                <div style={{background:"#F2EDE1",borderRadius:"16px 16px 16px 4px",padding:"0.75rem 1rem",display:"flex",gap:"4px",alignItems:"center"}}>
                  {[0,1,2].map(i=>(
                    <span key={i} style={{width:"6px",height:"6px",borderRadius:"50%",background:"#8F92AC",display:"inline-block",animation:`bounce 1.2s ease-in-out ${i*0.2}s infinite`}}/>
                  ))}
                </div>
              </div>
            )}
            ...
            <div style={{padding:"0 0.75rem 0.75rem",display:"flex",flexWrap:"wrap",gap:"0.375rem"}}>
              {suggestions.map(s=>(
                <button key={s} onClick={()=>{setInput(s);}} style={{
                  background:"rgba(41,171,226,0.08)",border:"1px solid rgba(27,117,187,0.3)",borderRadius:"999px",
                  padding:"0.3125rem 0.75rem",fontSize:"0.75rem",color:"#1B75BB",
          ...
          {/* Input */}
          <div style={{padding:"0.75rem",borderTop:"1px solid rgba(35,39,111,0.12)",display:"flex",gap:"0.5rem"}}>
            <input
              value={input}
              ...
              style={{
                flex:1,background:"#FAF7F0",border:"1px solid rgba(35,39,111,0.2)",
                borderRadius:"10px",padding:"0.625rem 0.875rem",
                color:"#20234B",fontSize:"0.875rem",outline:"none",
              }}
            />
            <button onClick={send} disabled={!input.trim()||loading} style={{
              background:"#2E3192",border:"none",borderRadius:"10px",
